...
function Shipping() {
  ...
  // const [itemsCredentials, setItemsCredentials] = useState(); 
    ...
    const itemsCredentials = cartItems.map(cart => {
      return {id:cart.id , quantity:cart.quantity, totalCost:cart.amountByQuantity, title:cart.title, formats:cart.formats , category:cart.bookshelves[0].split(':').pop()}
    })    ; 
  ...
  function handleOnSubmit(e){
    ...
    inputWithoutShippingType.append('orderedItems', JSON.stringify(itemsCredentials)) 
    submit(inputWithoutShippingType, {method:'post', action:'/cart/shipping'})
  ...
  return (
    ...
    <div className="flex h-full w-full flex-col items-start justify-center gap-3 p-4" >
      <CartNavigation />
      <Form className="grid w-full lg:grid-cols-3 lg:gap-x-6" onSubmit={e=> handleOnSubmit(e)} method="post">
...
export async function actionSendOrderDetails({request, params}){
  ...
  // const {} = data; 
  const shippingAddress= {city:data.city, state:data.state, zipCode:data.zipCode, shippingType:data.shippingType, phoneNumber:data.phoneNumber}
  ...
  try {
    const {data} = await server.post('/order', {email, fullName ,description, shippingAddress , orderedItems}); 
    ...
    toast.error(error||error?.response || error?.message || 'Cannot Proceed with order'); 
    throw new Response(JSON.stringify(error?.message || 'Cannot Proceed with order'), {status:404})
